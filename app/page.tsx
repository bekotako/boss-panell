...
const DICTIONARY: any = {
  tr: {
    ...
    logout: "Çıkış Yap", loginTitle: "Boss Panel Başlatılıyor..."
  },
  en: {
    ...
    logout: "Logout", loginTitle: "Starting Boss Panel..."
  }
};
...
export default function Home() {
  ...
  const fetchData = async (userId: string) => {
    ...
    if (tData && tData.length > 0) setTransactions(tData); else loadLocalData();
    const { data: dData } = await supabase.from('debts').select('*').eq('user_id', userId).order('created_at', { ascending: false });
    if (dData && dData.length > 0) setDebts(dData);
  };
  ...
  const toggleTheme = () => { const newTheme = theme === 'light' ? 'dark' : 'light'; setTheme(newTheme); localStorage.setItem('theme', newTheme); if(newTheme === 'dark') document.documentElement.classList.add('dark'); else document.documentElement.classList.remove('dark'); };
  
  const handleLogout = async () => {
    if (user) { await supabase.auth.signOut(); router.replace('/login'); } else { alert("Offline mode active."); }
  };

  const addTransaction = async () => {
    if (!desc || !amount) return alert("Eksik bilgi");
    const dateFormatted = new Date(dateVal).toLocaleDateString(lang==='tr'?'tr-TR':'en-US');
    const newItem = { id: Date.now(), date: dateFormatted, description: desc, desc: desc, category, amount: parseFloat(amount), currency, user_id: user?.id };
    const updated = [newItem, ...transactions];
    setTransactions(updated); saveToLocal('transactions', updated);
    if (user) await supabase.from('transactions').insert([{ user_id: user.id, date: dateFormatted, description: desc, category, amount: parseFloat(amount), currency }]);
    setDesc(''); setAmount('');
  };

  const deleteTransaction = async (id: number) => {
    if(!confirm("Sil?")) return;
    const updated = transactions.filter(t => t.id !== id); setTransactions(updated); saveToLocal('transactions', updated);
    if (user) await supabase.from('transactions').delete().eq('id', id);
  };
  ...
  const addDebt = async () => {
    ...
    setDebts([...debts, newDebt]); saveToLocal('debts', [...debts, newDebt]);
    if(user) await supabase.from('debts').insert([{ user_id: user.id, name: debtName, type: debtType, amount: parseFloat(debtAmount), currency: debtCurrency, date: dDate }]);
    setDebtName(''); setDebtAmount('');
  };

  const deleteDebt = async (id: number) => { if(!confirm("Sil?")) return; const updated = debts.filter(d => d.id !== id); setDebts(updated); saveToLocal('debts', updated); if(user) await supabase.from('debts').delete().eq('id', id); };
  ...
  const applyRecurring = () => {
    ...
    const newTrans = recurringItems.map(r => ({ id: Date.now()+Math.random(), date: new Date().toLocaleDateString(), desc: r.desc, category: 'Fatura', amount: r.amount, currency: 'TRY', user_id: user?.id }));
    setTransactions([...newTrans, ...transactions]); saveToLocal('transactions', [...newTrans, ...transactions]); alert("İşlendi!");
  };

  const runAutoPilot = async () => {
    if (!apiKey) return alert(t.ai.warning); setLoadingAI(true); setAiResult('');
    ...
    const recent = transactions.slice(0, 5).map(x => `${x.desc || x.description}: ${x.amount}`).join(', ');
    try {
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages: [{ role: "user", content: `Analyze (Language: ${lang}): Income:${inc}, Expense:${exp}. Trans:${recent}.` }] }) });
  ...
  async function fetchLiveRates() { const btn=document.getElementById('btnFetchRates'); if(btn) btn.innerHTML='...'; try { const res=await fetch('https://api.exchangerate-api.com/v4/latest/TRY'); const data=await res.json(); setRates({USD:(1/data.rates.USD).toFixed(2), EUR:(1/data.rates.EUR).toFixed(2), GBP:(1/data.rates.GBP).toFixed(2)}); saveToLocal('rates', rates); alert("OK"); } catch{ alert("Err"); } finally { if(btn) btn.innerHTML=t.settings.fetchLive; } }
  function saveRates() { saveToLocal('rates', rates); alert("OK"); }
  function saveApiAndPin() { saveToLocal('groqApiKey', apiKey); if(pin.length===4) saveToLocal('userPin', pin); alert("OK"); }
  function generateReport(type: string) {
    if(transactions.length===0) return alert("No Data");
    if(type === 'csv') { const csv = "\uFEFFDate;Category;Desc;Amount\n" + transactions.map(t => `${t.date};${t.category};${t.desc||t.description};${t.amount} ${t.currency}`).join('\n'); const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], {type:'text/csv;charset=utf-8;'})); link.download="Report.csv"; link.click(); }
    else { const tbody = document.getElementById('printTableBody'); if(tbody) { tbody.innerHTML = transactions.map(t => `<tr><td class="border p-2">${t.date}</td><td class="border p-2">${t.category}</td><td class="border p-2">${t.desc||t.description}</td><td class="border p-2 text-right">${t.amount} ${t.currency}</td></tr>`).join(''); window.print(); } }
  }
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 pt-20 md:pt-8 transition-all">
        {activeTab === 'dashboard' && (
          <div className="animate-fade-in space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm overflow-x-auto"><div className="flex justify-between mb-4"><h3 className="font-bold">{t.dash.lastTrans}</h3><div className="flex gap-2"><select onChange={(e)=>setFilterType(e.target.value)} className="p-1 text-xs border rounded dark:bg-slate-700"><option value="all">{t.filter.all}</option><option value="income">{t.filter.income}</option><option value="expense">{t.filter.expense}</option></select><select onChange={(e)=>setSortType(e.target.value)} className="p-1 text-xs border rounded dark:bg-slate-700"><option value="newest">{t.filter.dateNew}</option><option value="oldest">{t.filter.dateOld}</option></select></div></div><table className="w-full text-left min-w-[500px]"><thead className="bg-gray-50 dark:bg-slate-700 text-xs uppercase"><tr><th className="p-3">{t.form.date}</th><th className="p-3">{t.form.desc}</th><th className="p-3 text-right">{t.form.amount}</th><th className="p-3 text-right">Sil</th></tr></thead><tbody>{filteredTransactions.slice(0,10).map((t:any)=><tr key={t.id} className="border-b dark:border-slate-700"><td className="p-3">{t.date}</td><td className="p-3">{t.desc||t.description}</td><td className={`p-3 text-right font-bold ${t.amount>0?'text-green-500':'text-red-500'}`}>{t.amount} {t.currency}</td><td className="p-3 text-right"><button onClick={()=>deleteTransaction(t.id)} className="text-red-500"><Trash2 size={16}/></button></td></tr>)}</tbody></table></div>
          </div>
        )}
        {activeTab === 'debts' && (
             <div className="animate-fade-in grid grid-cols-1 lg:grid-cols-3 gap-6">
                 <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm h-fit">
                     <h3 className="font-bold mb-4">{t.debts.new}</h3>
                     <input value={debtName} onChange={(e)=>setDebtName(e.target.value)} placeholder={t.debts.person} className="w-full p-3 border rounded-xl dark:bg-slate-700 mb-2" />
                     <select value={debtType} onChange={(e)=>setDebtType(e.target.value)} className="w-full p-3 border rounded-xl dark:bg-slate-700 mb-2"><option value="alacak">{t.debts.receivable}</option><option value="borc">{t.debts.payable}</option></select>
                     <div className="flex gap-2 mb-4"><input type="number" value={debtAmount} onChange={(e)=>setDebtAmount(e.target.value)} placeholder={t.form.amount} className="w-2/3 p-3 border rounded-xl dark:bg-slate-700" /><select value={debtCurrency} onChange={(e)=>setDebtCurrency(e.target.value)} className="w-1/3 p-3 border rounded-xl dark:bg-slate-700"><option>TRY</option><option>USD</option></select></div>
                     <button onClick={addDebt} className="w-full bg-purple-600 text-white p-3 rounded-xl font-bold">{t.form.save}</button>
                 </div>
                 <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm overflow-x-auto"><table className="w-full text-left min-w-[500px]"><thead className="bg-gray-50 dark:bg-slate-700 text-xs uppercase"><tr><th className="p-3">{t.debts.person}</th><th className="p-3">{t.debts.type}</th><th className="p-3">{t.form.amount}</th><th className="p-3">Sil</th></tr></thead><tbody>{debts.map((d:any)=><tr key={d.id} className="border-b dark:border-slate-700"><td className="p-3">{d.name}</td><td className="p-3">{d.type}</td><td className="p-3 font-bold">{d.amount} {d.currency}</td><td className="p-3"><button onClick={()=>deleteDebt(d.id)} className="text-red-500"><Trash2 size={16}/></button></td></tr>)}</tbody></table></div>
             </div>
        )}
        {activeTab === 'ai' && <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl text-center"><Brain size={64} className="mx-auto text-indigo-500 mb-4"/><h2 className="text-2xl font-bold mb-4">{t.ai.title}</h2><button onClick={runAutoPilot} disabled={loadingAI} className="bg-orange-600 text-white px-6 py-2 rounded-xl">{loadingAI ? '...' : t.ai.btnAnalyze}</button>{aiResult && <div className="mt-4 prose dark:prose-invert" dangerouslySetInnerHTML={{__html:marked.parse(aiResult)}}/>}</div>}
        {activeTab === 'goals' && <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl"><h3 className="font-bold mb-4">{t.goals.title}</h3><p>{t.goals.target}: {monthlyTarget}</p><button onClick={openModal} className="bg-blue-600 text-white px-4 py-2 rounded">{t.goals.set}</button></div>}
        {activeTab === 'history' && <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl"><h3 className="font-bold mb-4">{t.menu.history}</h3>{transactions.map((t:any)=><div key={t.id} className="p-2 border-b dark:border-gray-700">{t.date} - {t.desc||t.description} : {t.amount}</div>)}</div>}
        {activeTab === 'guide' && <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl"><h3 className="font-bold mb-4">{t.guide.title}</h3><p>{t.guide.text1}</p></div>}
        {activeTab === 'settings' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><h3 className="font-bold mb-4">{t.settings.key}</h3><div className="flex gap-2"><input type="password" value={apiKey} onChange={(e)=>setApiKey(e.target.value)} className="w-full p-2 border rounded dark:bg-slate-700"/><button onClick={saveApiAndPin} className="bg-green-600 text-white px-4 rounded">Save</button></div></div>
...
function StatsCard({ title, value, color, icon }: any) { const c={green:"from-green-500 to-emerald-700", red:"from-red-500 to-rose-700", blue:"from-blue-500 to-indigo-700"}; return <div className={`bg-gradient-to-br ${c[color as keyof typeof c]} p-6 rounded-2xl shadow-lg text-white`}><div className="flex justify-between"><div><p className="text-white/80 text-sm font-medium uppercase">{title}</p><h3 className="text-3xl font-bold mt-1">{value}</h3></div><div className="bg-white/20 p-3 rounded-full">{icon}</div></div></div>; }