...
const DICTIONARY: any = {
  tr: {
    ...
    settings: { title: "Ayarlar", lang: "Dil Seçimi", key: "API Anahtarı", security: "Güvenlik (PIN)", reset: "Verileri Sıfırla", rates: "Kurlar (1 Birim = ? TL)", recurring: "Sabit Gider Şablonları", exportTitle: "Raporlama", fetchLive: "Canlı Kur Çek" },
  ...
  en: {
    ...
    ai: { title: "Financial Autopilot", btnAnalyze: "Start Auto-Analysis", waiting: "AI analyzing...", warning: "Enter API Key in Settings.", intro: "AI analyzes expenses and gives personalized advice." },
    guide: { title: "User Guide", sec1:"System", text1:"Data is stored in cloud.", sec2:"Security", text2:"Only you can access." },
    settings: { title: "Settings", lang: "Language", key: "API Key", security: "Security (PIN)", reset: "Reset Data", rates: "Rates (1 Unit = ? TRY)", recurring: "Recurring Templates", exportTitle: "Reporting", fetchLive: "Fetch Live" },
...
export default function Home() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // 2. Local Ayarları Yükle (SSR Hatasını Önlemek İçin)
      if (typeof window !== 'undefined') {
        ...
        setLang(l); setTheme(th); setApiKey(k); setMonthlyTarget(Number(mt)); setRates(r); setRecurringItems(rec); setPin(p);
        if (th === 'dark') document.documentElement.classList.add('dark');
        if (p) setIsLocked(true);
        setDateVal(new Date().toISOString().split('T')[0]);
      }

      // 3. Veritabanından Verileri Çek
  ...
  const addTransaction = async () => {
    ...
    // Optimistik UI (Hemen ekranda göster, sonra kaydet)
    setTransactions([newItem, ...transactions]);
    
    const { error } = await supabase.from('transactions').insert([{ user_id: user.id, date: dateFormatted, description: desc, category, amount: parseFloat(amount), currency }]);
  ...
  const deleteTransaction = async (id: number) => {
    ...
    setTransactions(transactions.filter(t => t.id !== id)); // UI'dan sil
    await supabase.from('transactions').delete().eq('id', id); // DB'den sil
  };
  ...
  const applyRecurring = async () => {
    if(recurringItems.length === 0) return alert("Şablon yok.");
    if(!confirm("Eklensin mi?")) return;
  ...
  const runAutoPilot = async () => {
    ...
    const prompt = lang === 'tr' ? `Analiz et: Gelir ${inc}, Gider ${exp}. İşlemler: ${recent}.` : `Analyze: Income ${inc}, Expense ${exp}. Trans: ${recent}.`;
    try {
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages: [{ role: "user", content: prompt }] }) });
      const data = await res.json(); setAiResult(data.choices?.[0]?.message?.content || "Error");
    } catch { setAiResult("API Error"); } finally { setLoadingAI(false); }
  };

  const updateTarget = (val: string) => { setMonthlyTarget(Number(val)); saveToLocal('monthlyTarget', val); };
  ...
  // --- EXTRAS ---
  const fetchLiveRates = async () => { const btn=document.getElementById('btnFetchRates'); if(btn) btn.innerHTML='...'; try { const res=await fetch('https://api.exchangerate-api.com/v4/latest/TRY'); const data=await res.json(); const r = { USD: parseFloat((1/data.rates.USD).toFixed(2)), EUR: parseFloat((1/data.rates.EUR).toFixed(2)), GBP: parseFloat((1/data.rates.GBP).toFixed(2)) }; setRates(r); saveToLocal('rates', r); alert("OK"); } catch{ alert("Hata"); } finally { if(btn) btn.innerHTML=t.settings.fetchLive; } };
  const saveRates = () => { saveToLocal('rates', rates); alert("OK"); };
  const saveApiAndPin = () => { saveToLocal('groqApiKey', apiKey); if(pin.length===4) saveToLocal('userPin', pin); alert("OK"); };
  
  const exportToExcel = () => {
    if (transactions.length === 0) return alert("No data");
    let csv = "\uFEFFDate;Category;Desc;Amount\n";
    transactions.forEach(t => csv += `${t.date};${t.category};${t.description?.replace(/;/g, " ")};${t.amount} ${t.currency}\n`);
  ...
  const printReport = () => {
    if (typeof window !== 'undefined') {
        // Print işlemi için CSS class ekle
        ...
        if(tbody) {
            ...
            transactions.forEach(t => {
                tbody.innerHTML += `<tr class="border-b"><td class="p-2 border-r">${t.date}</td><td class="p-2 border-r">${t.category}</td><td class="p-2 border-r">${t.description}</td><td class="p-2 text-right font-bold">${t.amount} ${t.currency}</td></tr>`;
            });
        }
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-gray-800 dark:bg-slate-900 dark:text-gray-100 overflow-hidden font-sans">
      ...
      <div id="printArea" className="hidden">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8 text-center border-b pb-4">
            <div><p>INC</p><h3 id="printIncome"></h3></div>
            <div><p>EXP</p><h3 id="printExpense"></h3></div>
            <div><p>NET</p><h3 id="printNet"></h3></div>
        </div>
        <table className="w-full text-left border-collapse border"><thead><tr class="bg-gray-100"><th class="border p-2">Date</th><th class="border p-2">Cat</th><th class="border p-2">Desc</th><th class="border p-2">Amt</th></tr></thead><tbody id="printTableBody"></tbody></table>
      </div>
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition duration-200 ease-in-out z-40 flex flex-col`}>
        <div className="p-6 text-center border-b border-gray-200 dark:border-slate-700 hidden md:block"><h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600">BOSS v37</h1><p className="text-xs text-gray-400 mt-1 uppercase tracking-widest">Production Master</p></div>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 pt-20 md:pt-8 transition-all">
        ...
        {activeTab === 'ai' && <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm text-center animate-fade-in"><Brain size={64} className="mx-auto text-indigo-500 mb-4"/><h2 className="text-3xl font-bold mb-2">{t.ai.title}</h2><p className="text-gray-500 mb-6 max-w-md mx-auto">{t.ai.intro}</p><button onClick={runAutoPilot} disabled={loadingAI} className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-4 rounded-2xl shadow-xl hover:scale-105 transition font-bold text-lg flex items-center gap-3 mx-auto disabled:opacity-50">{loadingAI ? <RefreshCw className="animate-spin"/> : <Brain />} {loadingAI ? '...' : t.ai.btnAnalyze}</button>{aiResult && <div className="mt-8 text-left max-w-3xl mx-auto bg-gray-50 dark:bg-slate-900 p-8 rounded-2xl border dark:border-slate-700 prose dark:prose-invert" dangerouslySetInnerHTML={{ __html: marked.parse(aiResult) }} />}</div>}
        ...
        {activeTab === 'settings' && <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in"><div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><h3 className="font-bold mb-4">{t.settings.key} (Groq)</h3><div className="flex gap-2"><input type="password" value={apiKey} onChange={(e)=>setApiKey(e.target.value)} className="w-full p-2 border rounded-lg dark:bg-slate-700" /><button onClick={()=>saveApiAndPin()} className="bg-green-600 text-white px-4 rounded-lg"><Save/></button></div></div><div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border-l-4 border-indigo-500"><h3 className="font-bold mb-4">{t.settings.recurring}</h3><div className="flex gap-2 mb-4"><input placeholder="Kira" value={recDesc} onChange={(e)=>setRecDesc(e.target.value)} className="w-full p-2 border rounded dark:bg-slate-700"/><input placeholder="-5000" value={recAmount} onChange={(e)=>setRecAmount(e.target.value)} type="number" className="w-24 p-2 border rounded dark:bg-slate-700"/><button onClick={addRecurring} className="bg-indigo-600 text-white px-4 rounded">+</button></div><ul>{recurringItems.map((r:any)=><li key={r.id} className="flex justify-between text-sm p-2 border-b dark:border-slate-700"><span>{r.desc} ({r.amount})</span><button onClick={()=>deleteRecurring(r.id)} className="text-red-500">x</button></li>)}</ul></div><div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><h3 className="font-bold mb-4">{t.settings.rates}</h3><div className="flex gap-2"><button onClick={fetchLiveRates} id="btnFetchRates" className="flex-1 bg-blue-600 text-white py-2 rounded-lg text-sm">{t.settings.fetchLive}</button><button onClick={saveRates} className="flex-1 bg-yellow-600 text-white py-2 rounded-lg text-sm">{t.form.save}</button></div></div><div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><h3 className="font-bold mb-4">{t.settings.exportTitle}</h3><div className="flex gap-2"><button onClick={exportToExcel} className="flex-1 bg-green-600 text-white py-2 rounded-lg flex items-center justify-center gap-2"><Download size={16}/> Excel</button><button onClick={printReport} className="flex-1 bg-gray-600 text-white py-2 rounded-lg flex items-center justify-center gap-2"><Printer size={16}/> PDF</button></div></div><div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><h3 className="font-bold mb-4">{t.settings.security}</h3><div className="flex gap-2"><input type="password" value={pin} onChange={(e)=>setPin(e.target.value)} maxLength={4} className="w-full p-2 border rounded-lg dark:bg-slate-700" placeholder="PIN"/><button onClick={saveApiAndPin} className="bg-blue-600 text-white px-4 rounded-lg"><Lock/></button></div></div></div>}
