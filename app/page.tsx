...
export default function Home() {
  ...
  const handleLogout = async () => {
    if (user) {
        ...
        alert("Offline moddasınız. Veriler sadece bu cihazda.");
    }
  };
  ...
  const deleteDebt = async (id: number) => {
    ...
    if(user) await supabase.from('debts').delete().eq('id', id);
  };
  ...
  // SLIDER MANTIĞI (REACT STATE İLE GÜNCELLENDİ)
  const handleSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = Number(e.target.value);
    setMonthlyTarget(val);
    saveToLocal('monthlyTarget', val);
  };
  ...
  function openModal() { document.getElementById('targetModal')?.classList.remove('hidden'); } 
  function closeModal() { document.getElementById('targetModal')?.classList.add('hidden'); }
  function saveTarget() { saveToLocal('monthlyTarget', monthlyTarget); closeModal(); }
  function updateGoal(net: number) {
     // Bu fonksiyon artık sadece hesaplama için, görsel güncelleme React render ile yapılıyor
  }
  ...
  function generateReport(type: string) {
    ...
    if(type === 'csv') { const csv = "\uFEFFDate;Category;Desc;Amount\n" + transactions.map(t => `${t.date};${t.category};${t.description||t.desc};${t.amount} ${t.currency}`).join('\n'); const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], {type:'text/csv;charset=utf-8;'})); link.download="Report.csv"; link.click(); }
    else { 
        const tbody = document.getElementById('printTableBody'); 
        if(tbody) {
            tbody.innerHTML = transactions.map(t => `<tr><td class="border p-2">${t.date}</td><td class="border p-2">${t.category}</td><td class="border p-2">${t.desc||t.description}</td><td class="border p-2 text-right">${t.amount} ${t.currency}</td></tr>`).join(''); 
            document.getElementById('printDate')!.innerText = new Date().toLocaleDateString();
            document.getElementById('printIncome')!.innerText = totalIncome.toLocaleString();
            document.getElementById('printExpense')!.innerText = totalExpense.toLocaleString();
            document.getElementById('printNet')!.innerText = netProfit.toLocaleString();
            window.print(); 
        }
    }
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-gray-800 dark:bg-slate-900 dark:text-gray-100 overflow-hidden font-sans">
      <div id="printArea" className="hidden"><h1 className="text-3xl font-bold mb-4 text-center">REPORT</h1><p id="printDate" className="text-center mb-4"></p><div className="grid grid-cols-3 text-center mb-4 font-bold"><div>GELİR: <span id="printIncome"></span></div><div>GİDER: <span id="printExpense"></span></div><div>NET: <span id="printNet"></span></div></div><table className="w-full text-left border-collapse border"><thead><tr class="bg-gray-100"><th class="border p-2">Date</th><th class="border p-2">Cat</th><th class="border p-2">Desc</th><th class="border p-2">Amt</th></tr></thead><tbody id="printTableBody"></tbody></table></div>
