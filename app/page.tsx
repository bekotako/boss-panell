...
export default function Home() {
  ...
  const applyRecurring = async () => {
    ...
    setTransactions([...newTrans, ...transactions]); await supabase.from('transactions').insert(newTrans); alert("İşlendi!");
  };

  // --- AI ---
  const runAutoPilot = async () => {
    // Server-side API kullanıldığı için frontend API key'e gerek yok.
    // Sadece Vercel Env Variables kontrolü yapılmalı.
    ...
    } catch (e: any) { setAiResult("Hata: " + e.message); } finally { setLoadingAI(false); }
  };
  ...
  function generateReport(type: string) {
    ...
    else { const tbody = document.getElementById('printTableBody'); if(tbody) { tbody.innerHTML = transactions.map(t => `<tr><td class="border p-2">${t.date}</td><td class="border p-2">${t.category}</td><td class="border p-2">${t.description}</td><td class="border p-2 text-right">${t.amount} ${t.currency}</td></tr>`).join(''); document.getElementById('printDate')!.innerText = new Date().toLocaleDateString(); window.print(); } }
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-gray-800 dark:bg-slate-900 dark:text-gray-100 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition duration-200 ease-in-out z-40 flex flex-col`}>
        <div className="p-6 text-center border-b border-gray-200 dark:border-slate-700 hidden md:block"><h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600">BOSS v29</h1><p className="text-xs text-gray-400 mt-1 uppercase tracking-widest">Stable</p></div>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 pt-20 md:pt-8 transition-all">
        ...
        {activeTab === 'settings' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><h3 className="font-bold mb-4">{t.settings.security}</h3><div className="flex gap-2"><input type="password" value={pin} onChange={(e)=>setPin(e.target.value)} maxLength={4} className="w-full p-2 border rounded-lg dark:bg-slate-700 dark:text-white" placeholder="PIN"/><button onClick={savePin} className="bg-blue-600 text-white px-4 rounded-lg"><Lock/></button></div></div>
            </div>
        )}
      </main>
      {isModalOpen && <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 no-print"><div className="bg-white dark:bg-darkCard p-6 rounded-2xl w-full max-w-md border border-gray-200 dark:border-gray-700"><h2 className="text-xl font-bold mb-4 text-center dark:text-white">{t.goals.set}</h2><div className="flex justify-between mb-2 dark:text-white"><span id="modalTargetDisplay">{monthlyTarget} TL</span></div><input type="range" min="1000" max="100000" step="500" value={monthlyTarget} onChange={(e)=>{setMonthlyTarget(Number(e.target.value)); saveToLocal('monthlyTarget',e.target.value)}} className="slider w-full mb-4"/><div className="flex gap-2"><button onClick={saveTarget} class="flex-1 bg-blue-600 text-white py-2 rounded-lg">{t.form.save}</button><button onClick={closeModal} class="flex-1 bg-gray-200 text-gray-700 py-2 rounded-lg">{t.cancel}</button></div></div></div>}
    </div>
  );
}
